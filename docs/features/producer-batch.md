...
- **XML Element** list:
  1. `/kafkaRecords/record/headers/header`
  2. `/kafkaRecords/record`

This ensures that multiple `<record>` and multiple `<header>` elements are properly wrapped into JSON arrays `[...]`. The Kafka Adapter will automatically detect the resulting BadgerFish structure for headers and unpack it into clean Kafka key-value headers.
