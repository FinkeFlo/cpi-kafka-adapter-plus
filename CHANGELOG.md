...
## [Unreleased]
### Fixed
- Fixed the actual cause of `Node Crashed` on a plaintext Security Protocol against a TLS-only broker: the node is not hung, it is killed. A TLS listener answers a plaintext Kafka request with a TLS alert record beginning `15 03 03 00 02`; Kafka reads those first four bytes as the frame length, sees `0x15030300` and allocates 352,321,280 bytes — once per connection attempt, and the client keeps reconnecting. A captured heap dump shows the ~2 GB node heap at 95% before Cloud Foundry's `jvmkill` agent killed the JVM 31 s later. No timeout or exception handler in the adapter can prevent this: the allocation happens in Kafka's own network thread, the size is not configurable (the Kafka clients build their `Selector` with an unlimited receive size), and the process is killed from the outside. The adapter now detects the TLS listener before any Kafka client is created — it offers a TLS handshake on the broker port and refuses to start only when the peer demonstrably answers as a TLS server (handshake completed, or failed because its certificate is not trusted). Anything else — a plaintext listener, a connection error, a timeout — is inconclusive and changes nothing, so Kafka still reports its own connection problems. The probe runs on the producer, the transactional producer and the consumer, and only for plaintext protocols, so a correctly configured TLS connection does not pay for it.
...
- `scripts/build-esa.sh` now removes `target/` on the host before starting the container. macOS re-creates `.DS_Store` inside a directory as soon as its contents change, so the container's `mvn clean` could delete the file, find the directory non-empty again an instant later and abort the build with `Failed to clean project: Failed to delete /workspace/target/...`. Build tooling only, no effect on the adapter.
