...
- Dependabot pull requests now carry the `no-changelog` label. Dependabot cannot write a changelog entry, so without it every dependency pull request was born red on the changelog gate and needed a manual label before it could merge. A dependency bump with user-visible effect still needs an entry — write one and drop the label.
