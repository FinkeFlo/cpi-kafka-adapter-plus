...
## [1.3.6] - 2026-09-21
### Fixed
- Every adapter update lost the messages of every running Kafka integration flow on the tenant until the flow was redeployed. Deploying a new adapter version makes CPI purge the *old* bundle revision immediately, while the running routes keep its class loader; the next class or resource that had never been loaded before then fails (`NoClassDefFoundError … bundle wiring … no longer valid` on the first LZ4 batch or on `close()`, `SnappyError FAILED_TO_LOAD_NATIVE_LIBRARY` on the first snappy batch) and stays failed, because HotSpot caches the resolution failure. The adapter's own reaction made it worse: rebuilding the consumer and restarting the Camel route re-used the same dead class loader, so it looped every ~6 s and never left the consumer group cleanly. The adapter now pre-loads its whole class space once per bundle revision when the first endpoint starts — its own classes synchronously, one compress/decompress round trip per codec so the snappy/zstd/lz4 native libraries are extracted while the revision is live, and every remaining class on a low-priority daemon thread (~6,900 classes, ~2 s on a tenant, finished before the first partition assignment) — and reports it as `class-warmup.*`. Verified on a tenant by updating the adapter under a running flow and then sending LZ4 and snappy batches and undeploying the flow: no failure, clean `LeaveGroup`. The recovery loop is gone; a poll that still fails on a class, link or native-library load on a purged revision now stops the consumer once with `poll.bundle-wiring-invalid … action=redeploy the integration flow` and reports the state to the monitor, instead of retrying forever. On the send side there is no polling loop to stop, so every exchange that hits the dead class space now fails with an `IllegalStateException` naming the cause and the remedy ("Redeploy the integration flow …") instead of a generic send error, and the trace carries one `send.bundle-wiring-invalid` line per minute. Each start line names its bundle revision (`bundleId`, `bundleLastModified`, `stale`), so an affected flow can be identified from the trace. See issue #148.
- The class warm-up stopped at the bundle's own content, so classes the adapter borrows from *other* bundles were still loaded lazily and could still break a running flow after an adapter update. The bundle declares `DynamicImport-Package: *`; Camel, the SAP APIs, SLF4J and the `javax.*` packages are therefore wired on first reference. A reference that had never been reached while the revision was live — typically a Camel exception type or a helper on an error or stop path — went through the purged revision's wiring and failed with the same `bundle wiring … no longer valid` error that #148 fixed for everything inside the bundle. The warm-up now has a third stage: it reads the constant pool of every class the bundle ships and pre-loads the 220 classes that the bytecode references but the bundle does not contain, so nothing is left to resolve over a wire that an update can invalidate. It runs on the same low-priority daemon thread, after the existing stage 2, and costs roughly a second per bundle revision. `java.*` is left out deliberately: the OSGi specification requires every bundle class loader to delegate it to the parent, so those classes never travel over a bundle wire. Classes of optional dependencies that CPI does not provide (gRPC and jose4j) still fail, by design and by name, exactly as in stage 2. The build asserts the size and the namespaces of that set, so a new dependency cannot quietly widen what the adapter depends on at runtime. See issue #154.

### Changed
- `THIRD-PARTY.txt` is no longer committed; it is generated by license-maven-plugin during `generate-resources` and staged into the ADK build at package time, so every build already ships a freshly generated copy. Committing it therefore added no compliance value, only drift: Dependabot bumps changed the resolved versions without regenerating the file, so the build check added in 1.3.0 failed on every dependency PR and blocked the whole dependency pipeline. `pom.xml` now fails the build if the file is missing when the adapter project is staged, because an Ant `<include>` skips a missing file silently and the adapter would otherwise ship without its licence list.
- Dependabot pull requests now carry the `no-changelog` label, and the build workflow reacts to label changes. Dependabot cannot write a changelog entry, so without the label every dependency pull request was born red on the changelog gate and blocked. Applying the label by hand did not help either: `pull_request` does not report `labeled` events by default, so the gate — which reads the labels from the event payload — kept its old result until the next push, although its own failure message points at the label as the remedy. A dependency bump with user-visible effect still needs an entry — write one and drop the label.
- The adapter no longer ships `commons-compress` and `commons-codec` (1.7 MB less, a 9.6% smaller `.esa`). Both entered the bundle only as transitive dependencies of Avro: the sole references are `org.apache.avro.file.XZCodec` and `BZip2Codec`, the codecs of the Avro object-container-file format. The adapter encodes single records in the Confluent wire format and never opens a data file, so no class in the bundle reaches them — verified by scanning every class in the built bundle, root content and every `Bundle-ClassPath` jar. Removing them also removes the 11 pack200 classes that could not be loaded during the class warm-up. Avro, message encoding and decoding are unchanged. See issue #151.
...
- The live 1.3 metadata files are now named after their line's baseline, `metadata-{sender,receiver}-1.3.0.xml`. They carried the 1.3 definitions under a `-1.2.0.xml` name — a leftover of the 1.3.0 release, which raised the 1.2 files in place instead of adding new ones; 1.3.5 restored the 1.2 line under `-1.2.8.xml` but left the misleading name behind. A filename that claims a different line than its content is a trap when freezing or copying a variant. This is a pure rename: the bundle still ships eight metadata files, the variant identity lives in `version::` inside the file and is unchanged, and the ADK's `check` goal confirms no duplicate variant. Older files keep their historic names — renaming them would rewrite the filename-keyed records of the frozen-metadata guard for cosmetic gain. `VERSIONING.md` now states the convention for new lines, and a build guard asserts that every metadata file's name declares the same line as its content. The guard compares `MAJOR.MINOR` only: the micro in the name is the line's baseline and drifts from the running micro by design, so checking it would fight the convention instead of protecting it. The old state fails it by name.
