...
- MPL error diagnostics now include the full Java stack trace in both trace messages and `KafkaAdapterError` attachments (instead of a bounded top-stack summary), and attachment writing is now configurable via `writeMplErrorAttachment` (default `true`) on both sender and receiver channels.
